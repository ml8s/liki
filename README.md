...
- **给你的价值**：一个结论，四种保障——排盘不靠模型（天文历算引擎）、断语不靠发挥（46 张真值表 656 条断语全部带经典原文）、流程不许跳步（关卡制检查表）、结论可用你的人生事件复核（考时校准）。
...
**② 断语表驱动，不靠模型发挥** — 46 张断语真值表（八字 26 + 紫微 20，共 **656 条断语**），每条带**经典原文列**（《渊海子平》《子平真诠》《滴天髓》《三命通会》《紫微斗数全书》等）——规则就是规则，可读、可改、可评审，不存在"参考"弱化。流年另有 7 张 yearly 断语表（应期/婚姻/六亲/财运/事业/健康/学业/子女，含值年神煞）。
...
├─ 断语层  tools/ 46 张真值表（656 条断语＋经典原文）＋ 因子引擎（497 行因子定义）
