...
- 1.17.0: 子包取消独立版本号，统一由根版本控制；README 白描改写+英文版；根 SKILL 路由前调 rpc.discover，子 SKILL 全部删除方法表/参数收集/数据来源（由 schema 驱动）；问卦新增输出规则（结论先行）+ 三份报告模板；起名重构双路径+报告精简+异常去混淆；命理用神方法论独立成节（权重聚合单列）+ 元余描述精简；知识根基四书拆行；VERSION 文件同步根版本
