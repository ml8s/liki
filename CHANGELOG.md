# Changelog
## 3.8.1（未发布）——窄表 gen_factors 往返 bug 修复 + 得地因子判定恢复
...
## 3.8.0（未发布）——agent 综合裁决层修复（EXEC 37 题 + MISS 部分）
...
## 3.6.2（未发布）——评测统一（判题唯一走 skill-up agent——eval_hybrid 降级为数据检查）
...
## 3.6.1（未发布）——去异化补齐 + 评审修复（should-fix 全清）
...
## 3.6.0（未发布）——综合评定回归 agent（评测标签移出 skill——去异化）
...
## 3.5.0（未发布）——命理逻辑全表化（Python 建表+查表——零命理定义）
...
## 3.4.2（未发布）——tools 去 mingli（推理机根 + 术数子目录）
...
## 3.4.1（未发布）——断语表 csv 归 tools（谁用归谁）
...
## 3.3.9（未发布）——mingli 归 tools（工具层——推理机）
...
## 3.3.5（未发布）——constants 单一来源（清代码硬编码重复）
...
## 3.2.1（未发布）——断语表真值表化（纯取值——与/或/非全用行表达）
...
## 3.2.0（未发布）——唯一入口 evaluator（排盘/推演/断命收敛）
...
## 3.1.1（未发布）——喜忌扩展至观察域（凶兆=忌神为患/吉兆=用神受助）
...
## 3.0.9（未发布）——抄 6 专项断事表（mingli-skills phase6 经典断语入 yaml）
...
## 3.0.8（未发布）——断语权重回归命理（约束补全上下文——去标签/去程序排序）
...
## 3.0.7（未发布）——架构归类澄清 + constants 恢复（复合因子层字典）
...
## 3.0.6（未发布）——domains 断语 md 信息保全 + 补规则
...
## 3.0.3（未发布）——数据库约束（schema 校验 + 视图一致性）
