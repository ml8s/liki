...
//   palaceIndex       : 命宫序，0=命宫 ... 11=父母 —— 仅用于输出层
// 安星/流盘内部计算统一用 zhiIdx；palaceIndex 只在 Chart.Palaces 输出时生成。
// 转换关系：
//   zhiIdx → Zhi        : +1
//   Zhi → zhiIdx        : -1
//   anXingIdx → zhiIdx  : (anXingIdx + 2) % 12   （寅=0 → 子=2）
//   zhiIdx → anXingIdx  : (zhiIdx - 2 + 12) % 12
//   zhiIdx → palaceIndex（命宫支锚定）: 见 palace_map.go zhiIdxToPalaceIndex
...
func buildFlowPalaces(liuPanIdx int, starByAnXingIdx map[int][]string) [12]flowPalace {
	...
	for i := 0; i < 12; i++ {
		...
		// 流盘宫名 = palaceLabels（根）的反向索引（顺时针序）：
