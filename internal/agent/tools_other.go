...
func qimenChartHandler(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
	...
	if p.QianShi != "" {
		// 带占事 → 聚合用神
		q, err := qimen.ParseQianShi(p.QianShi)
		if err != nil {
			return nil, fmt.Errorf("qimen.chart: %w", err)
		}
		result := qimen.ComputeChartWithYongShen(st, kind, q, p.Gender, p.BirthYear)
