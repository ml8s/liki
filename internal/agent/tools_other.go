...
func liuyaoPatternsHandler(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
	var p struct {
		SolarTime string `json:"solar_time"`
		YongShen  string `json:"yong_shen"`
		Yaos      [6]int `json:"yaos"`
	...
	if err := json.Unmarshal(raw, &p); err != nil {
		return nil, fmt.Errorf("liuyao.patterns: %w", err)
	}
	for i, v := range p.Yaos {
		if v < 6 || v > 9 {
			return nil, fmt.Errorf("liuyao.patterns: yao[%d] = %d, must be 6-9", i, v)
		}
	}
	st, err := parseSolarTime(p.SolarTime)
	if err != nil {
	...
	patterns := liuyao.ComputePatterns(&chart, ys)
