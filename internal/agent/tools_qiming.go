...
var qimingMethods = []RPCMethod{
	{
		Name: "qiming.pick", Description: "按五行取可用起名字。一次取一个五行，用+喜需调两次。返回按笔画分组的字库。",
		Params: mustSchema(`{"type":"object","properties":{"wuxing":{"type":"string","enum":["木","火","土","金","水"],"description":"单个五行"}},"required":["wuxing"]}`),
	...
	{
		Name: "qiming.build", Description: "起名组名。传入过滤后的字库和可选的笔画约束对，全量排列生成候选名。",
	...
	{
		Name: "qiming.wuge", Description: "查五格可行笔画对。传入姓氏和字数，返回人/地/外/总四格全吉的笔画组合。",
	...
	{
		Name: "qiming.check", Description: "起名评估。批量评估名字，返回五格三才五行音韵全量判定。",
		...
		Result: envelopeSchema(`{"type":"array","items":{"type":"object","properties":{"name":{"type":"string"},"surname":{"type":"string"},"given_name":{"type":"string"},"characters":{"type":"array"},"wuge":{"type":"object","description":"五格配置","properties":{"tiange":{"type":"object","description":"天格"},"renge":{"type":"object","description":"人格"},"dige":{"type":"object","description":"地格"},"waige":{"type":"object","description":"外格"},"zongge":{"type":"object","description":"总格"}}},"sancai":{"type":"object","description":"三才五行","properties":{"tian_cai":{"type":"string","description":"天才五行"},"ren_cai":{"type":"string","description":"人才五行"},"di_cai":{"type":"string","description":"地才五行"}}},"phonetic":{"type":"object","properties":{"tones":{"type":"string"}},"required":["tones"]},"wuxing_match":{"type":"boolean"},"wuxing":{"type":"object","properties":{"yong":{"type":"boolean"},"xi":{"type":"boolean"},"ji":{"type":"boolean"}},"required":["yong"]}},"required":["name","surname","given_name","wuge","sancai","phonetic","wuxing_match"]}}`),
	},
	{
		...
		Result:  envelopeSchema(`{"type":"object","properties":{"char":{"type":"string"},"wuxing":{"type":"string"},"stroke":{"type":"integer"},"radical":{"type":"string"},"pinyin":{"type":"string"},"tone":{"type":"integer"}},"required":["char","wuxing","stroke","pinyin"]}`),
	},
}
