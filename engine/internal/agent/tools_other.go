...
var otherMethods = []RPCMethod{
	...
	},
	{
		...
		Params:  mustSchema(`{"type":"object","properties":{"period_date":{"type":"string","format":"date","description":"宅运起盘日期，YYYY-MM-DD；年份 1864-2200"},"zuo_shan":{"type":"integer","minimum":0,"maximum":23,"description":"坐山，二十四山 index：0=子,1=癸,2=丑,3=艮,4=寅,5=甲,6=卯,7=乙,8=辰,9=巽,10=巳,11=丙,12=午,13=丁,14=未,15=坤,16=申,17=庚,18=酉,19=辛,20=戌,21=乾,22=亥,23=壬"},"xiang_shan":{"type":"integer","minimum":0,"maximum":23,"description":"朝向；必须等于坐山后 12 山（180°相对）"}},"required":["period_date","zuo_shan","xiang_shan"]}`),
		Handler: xuankongChartHandler,
		Result:  envelopeSchema(`{"type":"object","properties":{"yun":{"type":"object","description":"三元九运","properties":{"year":{"type":"integer","description":"当前年份"},"yuan":{"type":"string","description":"上元/中元/下元"},"yun_number":{"type":"integer","description":"运数1-9"},"yun_name":{"type":"string","description":"运名:一运/九运"},"start_year":{"type":"integer","description":"本运起始年"}},"required":["year","yuan","yun_number"]},"gong_wei":{"type":"array"},"wang_shan":{"type":"boolean"},"chart_digest":{"type":"string","description":"canonical SHA-256 chart digest"},"zuo_shan":{"type":"integer","description":"坐山(0-23)"},"zuo_shan_name":{"type":"string","description":"坐山名，如子/乾"},"xiang_shan":{"type":"integer","description":"向山(0-23)"},"xiang_shan_name":{"type":"string","description":"向山名，如午/巽"},"shan_xing":{"type":"boolean","description":"双星会坐：坐宫山向星皆当令"},"wang_xiang":{"type":"boolean","description":"旺向：向宫向星=当令"},"xiang_xing":{"type":"boolean","description":"双星会向：向宫山向星皆当令"},"xing_jia_hui":{"type":"array","description":"星加会"},"shou_shan_chu_sha":{"type":"object","description":"收山出煞"},"fu_yin":{"type":"boolean","description":"伏吟（运盘）"},"xia_shui":{"type":"boolean","description":"上山下水：向宫山星=当令且坐宫向星=当令"}},"required":["yun","gong_wei","wang_shan"]}`),
	},
	{
		Name: "xuankong.liunian", Description: "玄空流年飞星。chart（可选）+ year → 流年飞星盘 + 宅盘凶星落宫对照（确定性计算）。",
		Params:  mustSchema(`{"type":"object","properties":{"chart":{"type":"object","required":["chart_digest"],"description":"可选：xuankong.chart 返回的未修改宅盘；chart_digest 必须匹配"},"year":{"type":"integer","description":"年份"}},"required":["year"]}`),
	...
	},
	{
		...
		Params:  mustSchema(`{"type":"object","properties":{"start_date":{"type":"string","format":"date","description":"起始日期 YYYY-MM-DD"},"count":{"type":"integer","minimum":1,"maximum":30,"description":"查几天，默认3，最多30"},"event":{"type":"string","enum":["wedding","engage","open","sign","move","travel","build","exam","medical","funeral"],"description":"事项类型；缺省仅列黄历事实"}},"required":["start_date"]}`),
