...
var qimingMethods = []RPCMethod{
	{
		Name: "qiming.pick", Description: "起名取字（合并五格计算）。wuge=true 时按姓氏和候选字的 Kangxi 笔画计算五格并取字（surname 必填）；wuge=false 时按现代笔画取字（surname 不需要）。count 恒生效：1=单名（每组合仅 first 第1字池）/2=双名（每组合 first 第1字池 + second 第2字池）。wuxing2 默认同 wuxing1（用+喜时传不同五行）。输出 combos 数组，每组合 {id, first, second}，first/second 为纯字数组。LLM 筛选后传给 qiming.build。",
	...
	},
	{
		...
		Result:  envelopeSchema(`{"type":"object","properties":{"char":{"type":"string"},"wuxing":{"type":"string","enum":["金","木","水","火","土"]},"stroke":{"type":"integer","description":"现代规范汉字笔画"},"kangxi_stroke":{"type":"integer","description":"Kangxi 形体对应的笔画；Wuge 计算使用该值"},"radical":{"type":"string"},"pinyin":{"type":"string"},"tone":{"type":"integer"},"traditional":{"type":"string","description":"繁体字"},"kangxi_form":{"type":"string","description":"笔画对应的 Kangxi 形体"}},"required":["char","wuxing","stroke","kangxi_stroke","pinyin"]}`),
	},
}
