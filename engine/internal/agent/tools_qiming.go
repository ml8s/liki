...
var qimingMethods = []RPCMethod{
	{
		Name: "qiming.pick", Description: "起名取字（合并五格计算）。wuge=true 时按姓氏和候选字的 Wuge/Kangxi 笔画计算吉笔画并取字（surname 必填）；wuge=false 时按现代笔画取字（surname 不需要）。count 恒生效：1=单名（每组合仅 first 第1字池）/2=双名（每组合 first 第1字池 + second 第2字池）。wuxing2 默认同 wuxing1（用+喜时传不同五行）。输出 combos 数组，每组合 {id, first, second}，first/second 为纯字数组。LLM 筛选后传给 qiming.build。",
	...
	},
	{
		...
		Result:  envelopeSchema(`{"type":"array","items":{"type":"object","properties":{"name":{"type":"string"},"surname":{"type":"string"},"given_name":{"type":"string"},"characters":{"type":"array","items":{"type":"object","properties":{"char":{"type":"string"},"wuxing":{"type":"string","enum":["金","木","水","火","土"]},"stroke":{"type":"integer","description":"现代规范汉字笔画"},"kangxi_stroke":{"type":"integer","description":"按 Kangxi 形体计算的 Wuge 笔画"},"kangxi_form":{"type":"string","description":"Wuge 笔画采用的 Kangxi 形体"},"pinyin":{"type":"string"},"tone":{"type":"integer"},"traditional":{"type":"string"}},"required":["char","wuxing","stroke","kangxi_stroke","pinyin"]}},"wuge":{"type":"object","description":"五格配置（wuge=false 时不返回）","properties":{"tiange":{"type":"object","description":"天格"},"renge":{"type":"object","description":"人格"},"dige":{"type":"object","description":"地格"},"waige":{"type":"object","description":"外格"},"zongge":{"type":"object","description":"总格"}}},"sancai":{"type":"object","description":"三才配置（wuge=false 时不返回）","properties":{"configuration":{"type":"string","description":"三才组合（如木火土）"},"ji_xiong":{"type":"string","description":"三才吉凶","enum":["大吉","吉","半吉","凶"]},"description":{"type":"string","description":"三才解释"}},"required":["configuration","ji_xiong"]},"phonetic":{"type":"object","properties":{"tones":{"type":"string"}},"required":["tones"]},"wuxing_match":{"type":"boolean"},"wuxing":{"type":"object","properties":{"yong":{"type":"boolean"},"xi":{"type":"boolean"},"ji":{"type":"boolean"}},"required":["yong"]}},"required":["name","surname","given_name","phonetic","wuxing_match"]}}`),
	},
	{
		Name: "qiming.char", Description: "查字。查询单个汉字的五行、现代笔画、Wuge/Kangxi 笔画、部首、拼音等信息。",
		...
		Result:  envelopeSchema(`{"type":"object","properties":{"char":{"type":"string"},"wuxing":{"type":"string","enum":["金","木","水","火","土"]},"stroke":{"type":"integer","description":"现代规范汉字笔画"},"kangxi_stroke":{"type":"integer","description":"按 Kangxi 形体计算的 Wuge 笔画"},"radical":{"type":"string"},"pinyin":{"type":"string"},"tone":{"type":"integer"},"traditional":{"type":"string","description":"繁体字"},"kangxi_form":{"type":"string","description":"Wuge 笔画采用的 Kangxi 形体"}},"required":["char","wuxing","stroke","kangxi_stroke","pinyin"]}`),
	},
}
