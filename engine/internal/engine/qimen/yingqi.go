...
func computeYingQi(pan pan) YingQi {
	...
	chongStrs := []string{ganzhi.ZhiName(chongZhi(maZhi))}
	...
	var kwZhiStrs []string
	for _, z := range kwZhi {
		kwZhiStrs = append(kwZhiStrs, ganzhi.ZhiName(z))
	}
	if len(kwZhiStrs) > 0 {
		yq.KongWang = "空亡在" + strings.Join(kwZhiStrs, " ") + "，填实或冲空之时应事"
	}
...
// chongZhi returns the opposing zhi (冲).
