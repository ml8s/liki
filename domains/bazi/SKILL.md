...
- **何时使用**：论命（性格/运势/健康/婚姻/事业/学业/财运）、大运流年吉凶、合盘、起名取用神
- 与紫微分流：八字=五行生克推演（旺衰/用神/格局/十神）；紫微=宫位星曜框架（格局/宫位/四化）；两者可交叉验证时辰

## 边界规则

- 时辰不明默认 12:00 并在报告中注明；出生地必须传经度（真太阳时校正，海外/西部命例必传否则时辰错）
- 用神由 LLM 综合三派（chart.yong_shen：扶抑/调候/格局）裁决，喜忌取同派配套值，不武断
- 断语不产出抽象评级档位（吉凶/分数），只给「符号→现实」翻译
- 合盘先执行「致命排除」（六冲/三刑/比劫重等），排除后仍匹配才论可持续性
...
| domains/bazi/fangfa/dayun.md | 大运流年（应期取舍） | 《滴天髓》 |
...
4. **考时确认**：按 `domains/bazi/fangfa/calibration.md` 执行时辰校准。
   - 八字结构性排除后仍有 ≥ 2 盘 → 📖搜索 `domains/ziwei/fangfa/calibration.md` → 紫微交叉验证
   - 确认时辰后继续。宝宝/青少年跳过此步。
