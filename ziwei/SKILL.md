---
name: liki-ziwei
description: 紫微斗数模块（组件）。排盘、星曜、宫位、四化、格局。
---

# 紫微斗数

## 知识索引

| 文件 | 功能 | 输入 | 输出 |
|------|------|------|------|
| knowledge/zhuxing.md | 主星性格 | 命宫主星 | 性格基调 |
| knowledge/fuxing.md | 辅星作用 | 辅星名称 | 吉凶作用 |
| knowledge/miaowang.md | 庙旺表 | 主星+宫位 | 星力强度 |
| knowledge/shengong.md | 身宫方向 | 身宫宫位 | 着力方向 |
| knowledge/gong12.md | 十二宫要点 | 宫位+星曜 | 解读要点 |
| knowledge/sihua.md | 四化规则 | 四化星+宫位 | 含义影响 |
| knowledge/geju.md | 特殊格局 | 星曜组合 | 格局名称+评分 |
| knowledge/gexing.md | 性格双盘 | 命宫主星+日主 | 性格描述 |

## 技术流程

### 命盘分析

1. 收集参数 → TimeSet
2. 排八字（仅四柱基础，不读八字方法论）
3. 排紫微盘（调用 `ziwei.chart`）
4. 排大限（调用 `ziwei.daxian`）
5. 解读：调 knowledge/ 各决策表 → 命宫→身宫→十二宫→四化→三方→格局→综合
6. 产出：结构化数据 → `app/fatechart.md` 输出

📖 搜索 knowledge/zhuxing.md → 读取主星表
📖 搜索 knowledge/fuxing.md → 读取辅星表
📖 搜索 knowledge/miaowang.md → 读取庙旺表
📖 搜索 knowledge/shengong.md → 读取身宫表
📖 搜索 knowledge/gong12.md → 读取十二宫表
📖 搜索 knowledge/sihua.md → 读取四化表
📖 搜索 knowledge/geju.md → 读取格局表
📖 搜索 knowledge/gexing.md → 读取性格表

### 合盘分析

1. 排双方紫微盘
2. 合盘（调用 `ziwei.bond`）
3. 解读：命宫对位→夫妻宫呼应→四化交集
4. 产出：结构化数据 → `app/compatibility.md` 输出

📖 搜索 knowledge/gong12.md → 读取夫妻宫要点

## 体系边界

- 紫微不串八字十神解释命盘
- 「同名陷阱」：七杀/太阳/太阴在两体系含义不同，不可混用
