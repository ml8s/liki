...
3. **收集**：逐步收集参数，缺什么问什么，一次只问 1-2 项。出生时间需精确到分（未知填0），出生城市需调 `city` 获取经纬度。
   ...
   - 合会冲刑：chart 对象（来自 `bazi.chart` 返回的 data）
   - 合盘：两套出生参数
...
chart → 读取 https://liki.hk/skills/report-chart.md，按其模板格式输出。bond → 读取 https://liki.hk/skills/report-bond.md。hehui 复用 report-chart.md 结构，叠加合会冲刑维度。liunian/liuyue/liuri/liushi/xiaoyun/xiaoxian 等 method 复用报告模板，叠加对应时间维度。
