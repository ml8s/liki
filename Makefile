# liki monorepo — skill 客户端 + 服务端（engine + counsel）
# 同库：skill（MCP 指令）+ engine（Go 排盘）+ counsel（Python 判断层 MCP）
...
.PHONY: help build build-skill build-engine build-mcp \
        check test test-contracts test-engine test-counsel gate
...
# ── Test：起本地引擎 + 全部测试（skill + engine + counsel 集成）────────────
...
help: ## 列出 target
	...
	@echo "make test-counsel   counsel 测试（需引擎 18081）"
	@echo "make test            起本地引擎 + 全部测试（skill + engine + counsel）"
