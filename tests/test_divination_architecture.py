...
TOOLS = ROOT / "analysis/liki_analysis/divination/tools"
...
def test_removed_orchestration_modules_are_gone():
    for name in (
        ...
        assert not (TOOLS / name if not name.startswith("app/") else ROOT / "analysis/liki_analysis/divination" / name).exists(), name
...
def test_snapshot_app_cards_replace_chart_cards():
    assert (ROOT / "analysis/liki_analysis/divination/app/liuyao-snapshot.md").is_file()
    assert (ROOT / "analysis/liki_analysis/divination/app/qimen-snapshot.md").is_file()
    assert not (ROOT / "analysis/liki_analysis/divination/app/liuyao-chart.md").exists()
    assert not (ROOT / "analysis/liki_analysis/divination/app/qimen-chart.md").exists()
...
def test_liuyao_static_hexagram_doc_separates_fact_from_action():
    text = (ROOT / "analysis/liki_analysis/divination/domains/liuyao/jixiong.md").read_text(encoding="utf-8")
