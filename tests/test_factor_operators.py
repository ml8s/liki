...
class TestOperators(unittest.TestCase):
    def test_现(self):
        ...
        fac = mock_factors(正财={"count": 0, "wuxing": "土"})
        self.assertEqual(_op("现", ["正财"], fac, "male", None), 0)

    def test_透(self):
        fac = mock_factors(正财={"tou_gan": True, "wuxing": "土"})
        self.assertEqual(_op("透", ["正财"], fac, "male", None), 1)
        fac = mock_factors(正财={"tou_gan": False, "wuxing": "土"})
        self.assertEqual(_op("透", ["正财"], fac, "male", None), 0)
    ...
    def test_克_五行生克(self):
        ...
        self.assertEqual(_op("克", ["财星", "印星"], fac, "male", None), 1)

    def test_克_不克(self):
        # 财（土）不克官（木）——土不克木 → 0
        fac = mock_factors(正财={"wuxing": "土", "count": 1}, 正官={"wuxing": "木", "count": 1})
        self.assertEqual(_op("克", ["财星", "官杀"], fac, "male", None), 0)
...
class TestOperatorsExtended(unittest.TestCase):
    ...
    def test_藏(self):
        ...
        fac = mock_factors(正印={"cang_zhi": False, "wuxing": "水"})
        self.assertEqual(_op("藏", ["正印"], fac, "male", None), 0)

    def test_有根(self):
        fac = mock_factors(正财={"has_root": True, "wuxing": "土"})
        self.assertEqual(_op("有根", ["正财"], fac, "male", None), 1)
        fac = mock_factors(正财={"has_root": False, "wuxing": "土"})
        self.assertEqual(_op("有根", ["正财"], fac, "male", None), 0)
    ...
    def test_弱_十神三条件(self):
        ...
        # 透干则不算弱
        fac = mock_factors(正财={"wuxing": "土", "tou_gan": True, "has_root": False})
        fac["wuxing"] = {"wang_shuai": {"土": "死"}}
        self.assertEqual(_op("弱", ["正财"], fac, "male", None), 0)
    ...
    def test_缺_五行(self):
        ...
        fac = mock_factors()
        fac["wuxing"] = {"count": {"木": 2, "火": 1, "土": 1, "金": 1, "水": 1}}
        self.assertEqual(_op("缺", ["土"], fac, "male", None), 0)
...
class TestDaYunOps_YearRange(unittest.TestCase):
    ...
    def test_大运窗口流年_年份段内(self):
        ...
        self.assertEqual(_liu_op("大运窗口流年", ["配偶星"], self._fac(), "male", None, ctx), 1)

    def test_大运窗口流年_窗口外(self):
        ctx = {"year": 1989, "liunian": {}}
        self.assertEqual(_liu_op("大运窗口流年", ["配偶星"], self._fac(), "male", None, ctx), 0)

    def test_大运窗口流年_非配偶星大运(self):
        # 丁卯=偏印运（非配偶星）——1995 窗口内但 shi_shen 不含配偶星 → 0
        fac = self._fac()
        ctx = {"year": 1995, "liunian": {}}
        self.assertEqual(_liu_op("大运窗口流年", ["配偶星"], fac, "male", None, ctx), 0)

    def test_换运流年_首年(self):
        ctx = {"year": 2001, "liunian": {}}
        self.assertEqual(_liu_op("换运流年", ["配偶星"], self._fac(), "male", None, ctx), 1)

    def test_换运流年_非首年(self):
        ctx = {"year": 2005, "liunian": {}}
        self.assertEqual(_liu_op("换运流年", ["配偶星"], self._fac(), "male", None, ctx), 0)
    ...
    def test_三刑_整组齐备(self):
        ...
        self.assertEqual(_liu_op("三刑", ["流年支"], mock_factors(), "male", ch, ctx), 1)

    def test_三刑_缺一组支(self):
        # 命局丑戌、流年寅——丑戌未缺未 → 0（旧 bug 会误命中）
        ctx = {"liunian": {"nian_zhi": "寅"}}
        ch = self._chart("戌", "丑", "子", "午")
        self.assertEqual(_liu_op("三刑", ["流年支"], mock_factors(), "male", ch, ctx), 0)
    ...
    def test_三刑_自刑单字(self):
        # 仅一个辰（无同字成双）→ 0（自刑不成立；旧 bug 会把"含辰"判为命中）
...
class TestLiuNianOps(unittest.TestCase):
    ...
    def test_流年值_命中与不命中(self):
        ...
        self.assertEqual(_liu_op("流年值", ["配偶星"], self._fac(), "male", chart, ctx), 1)
        ctx2 = {"liunian": {"nian_zhi": "丑"}, "chart": chart}
        self.assertEqual(_liu_op("流年值", ["配偶星"], self._fac(), "male", chart, ctx2), 0)

    def test_流年克_五行相克与不克(self):
        # 男命配偶星=正财（土）——流年支寅（木克土）→ 1；子（水不克土）→ 0
        fac = self._fac(shishen={"正财": {"wuxing": "土"}})
        ctx = {"liunian": {"nian_gan": "庚", "nian_zhi": "寅"}}
        self.assertEqual(_liu_op("流年克", ["配偶星"], fac, "male", None, ctx), 1)
        ctx2 = {"liunian": {"nian_gan": "庚", "nian_zhi": "子"}}
        self.assertEqual(_liu_op("流年克", ["配偶星"], fac, "male", None, ctx2), 0)

    def test_流年克_本命星不现仍可推导目标五行(self):
        # 甲男配偶星=财星土；本命财星虽不现，流年木仍克财星土。
        ctx = {"liunian": {"nian_gan": "甲", "nian_zhi": "寅"}}
        self.assertEqual(_liu_op("流年克", ["配偶星"], self._fac(), "male", None, ctx), 1)

    def test_旬空_空亡支命中与不命中(self):
        # 日柱甲子（甲子旬空戌亥）——流年支戌 → 1；午 → 0
        chart = self._chart(ri_gan="甲", ri_zhi="子")
        ctx = {"liunian": {"nian_zhi": "戌"}, "chart": chart}
        self.assertEqual(_liu_op("旬空", ["日柱", "流年支"], self._fac(), "male", chart, ctx), 1)
        ctx2 = {"liunian": {"nian_zhi": "午"}, "chart": chart}
        self.assertEqual(_liu_op("旬空", ["日柱", "流年支"], self._fac(), "male", chart, ctx2), 0)

    def test_旬空_非零偏移日柱(self):
        # 己亥日（甲午旬，空亡辰巳）——流年支辰 → 1（旧公式 diff=0 用例测不出，需非零偏移）
        ...
        self.assertEqual(_liu_op("旬空", ["日柱", "流年支"], self._fac(), "male", chart3, ctx3), 1)

    def test_干支相等_相同与不同(self):
        # 流年甲子 == 日柱甲子 → 1；日柱乙丑 → 0
        chart = self._chart(ri_gan="甲", ri_zhi="子")
        ctx = {"liunian": {"nian_gan": "甲", "nian_zhi": "子"}, "chart": chart}
        self.assertEqual(_liu_op("干支相等", ["流年", "日柱"], self._fac(), "male", chart, ctx), 1)
        chart2 = self._chart(ri_gan="乙", ri_zhi="丑")
        ctx2 = {"liunian": {"nian_gan": "甲", "nian_zhi": "子"}, "chart": chart2}
        self.assertEqual(_liu_op("干支相等", ["流年", "日柱"], self._fac(), "male", chart2, ctx2), 0)

    def test_引用本命_支持键与未知键(self):
        # 支持的 key 返回 ctx 值；未知 key → 0（死条件防护）
        ctx = {"snapshot": {"食伤旺": 1}}
        self.assertEqual(_liu_op("引用本命", ["食伤旺"], self._fac(), "male", None, ctx), 1)
        self.assertEqual(_liu_op("引用本命", ["未知键"], self._fac(), "male", None, ctx), 0)

    def test_流年宫化_无紫微四化数据(self):
        # 无 zw_liunian 数据 → 0（不得误命中）
        ctx = {"zw_liunian": {}}
        self.assertEqual(_liu_op("流年宫化", ["夫妻", "禄"], self._fac(), "male", None, ctx), 0)

    def test_流年透_配偶星透与不透(self):
        # 男命配偶星=正财/偏财——流年十神为正财 → 1；比肩 → 0
        ctx = {"liunian": {"shi_shen": "正财"}}
        self.assertEqual(_liu_op("流年透", ["配偶星"], self._fac(), "male", None, ctx), 1)
        ctx2 = {"liunian": {"shi_shen": "比肩"}}
        self.assertEqual(_liu_op("流年透", ["配偶星"], self._fac(), "male", None, ctx2), 0)

    def test_流年神煞_命中与不命中(self):
        ctx = {"liunian": {"shensha": [{"name": "红鸾"}, {"name": "天喜"}]}}
        self.assertEqual(_liu_op("流年神煞", ["红鸾"], self._fac(), "male", None, ctx), 1)
        ctx2 = {"liunian": {"shensha": [{"name": "桃花"}]}}
        self.assertEqual(_liu_op("流年神煞", ["红鸾"], self._fac(), "male", None, ctx2), 0)

    def test_年柱干伏吟_相同与不同(self):
        chart = self._chart()  # 年柱庚
        ctx = {"liunian": {"nian_gan": "庚"}, "chart": chart}
        self.assertEqual(_liu_op("年柱干伏吟", [], self._fac(), "male", chart, ctx), 1)
        ctx2 = {"liunian": {"nian_gan": "甲"}, "chart": chart}
        self.assertEqual(_liu_op("年柱干伏吟", [], self._fac(), "male", chart, ctx2), 0)

    def test_忌神干_命中与不命中(self):
        # 忌神=火（fu_yi.ji）——流年干丙（火）→ 1；庚（金）→ 0
        fac = self._fac()
        fac["yongshen"] = {"fu_yi": {"ji": "火"}}
        ctx = {"liunian": {"nian_gan": "丙"}}
        self.assertEqual(_liu_op("忌神干", [], fac, "male", None, ctx), 1)
        ctx2 = {"liunian": {"nian_gan": "庚"}}
        self.assertEqual(_liu_op("忌神干", [], fac, "male", None, ctx2), 0)
    ...
    def test_财坏印流年_印年被克(self):
        ...
        self.assertEqual(_liu_op("财坏印流年", [], fac, "male", None, ctx), 0)
        # 流年支戌（土）——土克水 → 1（流年干为印被流年支所克）
        ctx2 = {"liunian": {"nian_gan": "壬", "nian_zhi": "戌", "shi_shen": "正印"}}
        self.assertEqual(_liu_op("财坏印流年", [], fac, "male", None, ctx2), 1)
