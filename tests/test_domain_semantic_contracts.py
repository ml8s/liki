...
def test_family_wealth_loss_requires_unfavorable_peers():
    with (Path(__file__).resolve().parents[1] / "counsel/app/natal/tools/assertions/assertion_conditions.csv").open(
...
def test_factor_groups_do_not_duplicate_definitions():
    root = Path(__file__).resolve().parents[1] / "counsel/app/natal/tools/factors"
...
def test_factor_model_preserves_assertion_unconsumed_domain_fact():
    root = Path(__file__).resolve().parents[1] / "counsel/app/natal/tools"
...
def test_relation_assertions_do_not_overstate_transformation_success():
    ...
    path = Path(__file__).resolve().parents[1] / "counsel/app/natal/tools/assertions/assertions.csv"
...
def test_yearly_scene_aliases_have_matchable_domain_assertions():
    ...
    path = root / "counsel/app/natal/tools/assertions/assertions.csv"
    with path.open(encoding="utf-8-sig", newline="") as source:
        rows = list(csv.DictReader(source))
