...
class TestLiuNianOps(unittest.TestCase):
    ...
    def test_流年值_命中与不命中(self):
        ...
        chart = self._chart()
        ctx = {"liunian": {"nian_zhi": "子"}, "target": "配偶星", "chart": chart}
        self.assertEqual(_liu_op("流年值", ["目标星"], self._fac(), "male", chart, ctx), 1)
        ctx2 = {"liunian": {"nian_zhi": "丑"}, "target": "配偶星", "chart": chart}
        self.assertEqual(_liu_op("流年值", ["目标星"], self._fac(), "male", chart, ctx2), 0)

    def test_流年克_五行相克与不克(self):
        # 男命配偶星=正财（土）——流年支寅（木克土）→ 1；子（水不克土）→ 0
        fac = self._fac(shishen={"正财": {"wuxing": "土"}})
        ctx = {"liunian": {"nian_gan": "庚", "nian_zhi": "寅"}, "target": "配偶星"}
        self.assertEqual(_liu_op("流年克", ["目标星"], fac, "male", None, ctx), 1)
        ctx2 = {"liunian": {"nian_gan": "庚", "nian_zhi": "子"}, "target": "配偶星"}
        self.assertEqual(_liu_op("流年克", ["目标星"], fac, "male", None, ctx2), 0)

    def test_流年克_目标星无五行(self):
        # 目标星无 shishen 数据 → 0（不得误命中）
        ctx = {"liunian": {"nian_gan": "甲", "nian_zhi": "寅"}, "target": "配偶星"}
        self.assertEqual(_liu_op("流年克", ["目标星"], self._fac(), "male", None, ctx), 0)
    ...
    def test_引用本命_支持键与未知键(self):
        ...
        ctx = {"shi_shang_zhong": 1, "marriage_bad": 0, "shi_ke_guan": 0}
        self.assertEqual(_liu_op("引用本命", ["食伤重"], self._fac(), "male", None, ctx), 1)
        self.assertEqual(_liu_op("引用本命", ["未知键"], self._fac(), "male", None, ctx), 0)

    def test_流年宫化_无紫微四化数据(self):
        # 无 zw_liunian 数据 → 0（不得误命中）
        ctx = {"zw_liunian": {}, "target": "配偶星"}
        self.assertEqual(_liu_op("流年宫化", ["夫妻", "禄"], self._fac(), "male", None, ctx), 0)

    def test_流年透_目标星透与不透(self):
        # 男命配偶星=正财/偏财——流年十神为正财 → 1；比肩 → 0
        ctx = {"liunian": {"shi_shen": "正财"}, "target": "配偶星"}
        self.assertEqual(_liu_op("流年透", ["目标星"], self._fac(), "male", None, ctx), 1)
        ctx2 = {"liunian": {"shi_shen": "比肩"}, "target": "配偶星"}
        self.assertEqual(_liu_op("流年透", ["目标星"], self._fac(), "male", None, ctx2), 0)
