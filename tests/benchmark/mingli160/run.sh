...
LOCAL_MODEL_ENV="${LOCAL_MODEL_ENV:-tests/benchmark/mingli160/evals/.zhipu.local.env}"
...
ANSWER_FILES=(
  tests/benchmark/mingli160/answers.json
  tests/benchmark/mingli160/groups.json
  tests/benchmark/mingli160/cats.json
)
STASH_DIR="/tmp/liki-mingli160-answers"
...
LEFTOVER_YAML="$(ls tests/benchmark/mingli160/evals/.run-eval.*.yaml 2>/dev/null | head -1 || true)"
if [ -n "$LEFTOVER_YAML" ]; then
  echo "检测到上次评测残留临时配置——自动清理: $LEFTOVER_YAML"
  rm -f tests/benchmark/mingli160/evals/.run-eval.*.yaml
fi
...
ensure_local_engine
RUN_YAML="$(mktemp tests/benchmark/mingli160/evals/.run-eval.XXXXXX.yaml)"
sed -e "s|\${OPENAI_API_KEY}|$OPENAI_API_KEY|g" \
    ...
    tests/benchmark/mingli160/evals/eval.yaml > "$RUN_YAML"
...
if [ "$RESUME" = 1 ]; then
  ...
  LATEST_DONE="$(ls -dt tests/benchmark/mingli160/workspace/iteration-* 2>/dev/null | head -1 || true)"
...
skill-up run "$RUN_YAML" --parallelism "$PARALLELISM" --output-dir "$(pwd)/tests/benchmark/mingli160/workspace" --no-delete "${RESUME_ARGS[@]}"

# 2.5 失败 case 自动重跑（空输出 / 截断 / API 超时）——最多 1 次重试
LATEST_CHECK="$(ls -dt tests/benchmark/mingli160/workspace/iteration-* 2>/dev/null | head -1)"
if [ -n "$LATEST_CHECK" ]; then
  ...
  if [ ${#RETRY_CASES[@]} -gt 0 ]; then
    ...
    skill-up run "$RUN_YAML" --parallelism 4 --output-dir "$(pwd)/tests/benchmark/mingli160/workspace" --no-delete "${RETRY_ARGS[@]}" || true
  fi
fi
...
LATEST_ITER="$(ls -dt tests/benchmark/mingli160/workspace/iteration-* 2>/dev/null | head -1)"
if [ -n "$LATEST_ITER" ]; then
  ...
  echo "（答案判分已由 skill-up script judge（tests/benchmark/mingli160/grade-case.py）随评测完成——每盘正确率/错题见 $LATEST_ITER/benchmark.md 与 result.json 的 grading 段）"
else
  echo "未找到评测输出目录（tests/benchmark/mingli160/workspace/iteration-*），跳过判分"
fi
