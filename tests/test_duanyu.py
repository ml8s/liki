...
class TestEvaluateFactors(unittest.TestCase):
    ...
    def test_快照含关键因子(self):
        ...
        base = self._fac(正印={"count": 2, "transparent": True, "hidden": True, "timely": True, "rooted": True, "wuxing": "水"})
    ...
    def test_多遍稳定_确定性(self):
        base = self._fac(正官={"count": 1, "transparent": True, "timely": False, "wuxing": "木"})
...
class TestFactorMatchingContract(unittest.TestCase):
    ...
    def _fac(self, **ten_god_states):
        base = mock_base_context(**ten_god_states)
    ...
    def _chart(self, ri_gan="甲", yue_zhi="卯", state="帝旺"):
        return {
            ...
            "full": {
                "chang_sheng": [{"name": state, "index": yue_zhi}],
                "atomic_facts": {"month_longevity": state},
            },
        }

    def test_互斥枚举使用标量因子(self):
        base = self._fac()
        # 合并 base(ten_god_states/ri_gan) + _chart(chang_sheng/日支) 作为 chart
