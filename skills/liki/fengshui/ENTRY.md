...
本域无 Python 工具层。agent 只能使用 `fengshui/RPC.md` 声明的完整 JSON-RPC 报文和固定 discover scope；不得推断其他方法或参数。

## 路由

所有风水 / 家宅 / 布局 / 流年飞星问题读取 `fengshui/app/fengshui.md`，卡内再分八宅与玄空分支。

本领域无 Python 工具层；由 app 卡按固定 RPC discover 契约返回的方法调用引擎，并直接消费 RPC 结果。
