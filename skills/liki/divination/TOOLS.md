...
- 起卦创建快照：自动起卦 / 用户手摇（六组三枚正反）/ 爻值复现（六个爻值 6-9）。
  问一件事，参数按 schema 自举；`mode` 缺省自动；`matter` 与用神二选一。
...
- 排盘创建快照：普通策略问事（城市或经度+时间）/ 专占（`rule` 按 app 卡显式列出时才传）。
  参数按 schema 自举；`time` 缺省用服务端当前时间；`matter` 与用神二选一。
- 追问基于快照出断语：`snapshot` 必须用 `$QIMEN_SNAPSHOT` 原样对象，不得裁剪或重建。

## 3. 黄历择日

engine MCP 黄历工具：返回连续 N 天的黄历信息；`event` 给定时按建除事项规则输出
适配推荐。仅输出日期适配事实，不预测现实结果。

- `question` 必填；`event` 用 app 卡事项枚举中的英文值。
