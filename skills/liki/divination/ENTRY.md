...
> 覆盖六爻结果与应期、奇门策略与方向、黄历择日。LLM 负责语义路由和解释结构化因子；engine 负责排盘，counsel 负责判断与 answer 契约。

## 工具契约

divination 域通过 `counsel` MCP 连接器调用，不直接使用 RPC。

- 工具契约见 `divination/TOOLS.md`（工具参数由 MCP 连接器 schema 提供）
- 覆盖：六爻（起卦/追问）、奇门（排盘/追问）、黄历择日（engine 黄历）
...
| 长期命局 | 不临时起卦，改用八字命理技能 |
| 高风险现实事项 | 继续执行所选问卦流程；六爻/奇门附 `safety_advisory`，黄历附现实边界提示 |
...
- 高风险主题由共享安全规则识别为 `safety_advisory`（六爻/奇门）；黄历按 engine 择日事实输出并附现实边界提示。所有工具继续返回正常盘面、answer 或择日结果。
