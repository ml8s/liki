# Liki 八字紫微排盘 — 八字算命 / 紫微斗数 / 大运流年
>
> 覆盖八字算命、生辰八字排盘、四柱命盘、十神格局、大运流年，以及紫微斗数、紫微命盘、宫位星曜、大限流年；婚姻分析、八字合婚、事业分析、财运分析、学业分析、健康分析、性格分析、家庭六亲通过引擎断语合参。
...
| 1 | 时间距时辰交界 ≤5 分钟 | 先排盘，再提示相邻时辰校准 | 带校准提示的结果 |
...
| 5 | 双人关系 | `bond(pan_a, pan_b)` | 合盘原始事实 |
...
`query` 口径：`夫妻` / `官禄` / `财帛` 分别返回对应紫微宫位域；`十神` 返回八字结构及其跨人生领域信号；`用神` 除断语外额外返回 `fu_yi` / `tiao_hou` / `ge_ju` / `element_states` / `ten_god_states`。需要单一生活领域时优先使用宫位域，不把 `十神` 当作唯一领域查询。

## 硬边界

- `query` / `yearly_range` 只传入 `full_paipan` 的完整 `pan`；跨度含端点最多 120 年。
- `correct=true` 必须提供经度；缺出生地时先问城市，仍缺失则停止校正排盘。
- 出生时间距时辰交界 ≤5 分钟时，必须提示可校准；不得把该时辰表述为唯一确定事实。
