...
| 用户场景 | 交给 | 说明 |
| --- | --- | --- |
| 八字看命/流年/合盘/考时 | **liki-bazi**（八字专家）| 排盘 + 判断 + 合盘 + 考时（analysis-bazi）|
| 紫微看命/流年/合盘/考时 | **liki-ziwei**（紫微专家）| 排盘 + 判断 + 合盘 + 考时（analysis-ziwei）|
...
- 八字/紫微的**实际判断工具**由专家执行（analysis-bazi / analysis-ziwei），本域不直接调用。
