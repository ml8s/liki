...
> **本卡的 本命判断 / 应期判断 调用均传 `topics=["health"]`，仅保留健康域断语。**
...
| 2 | 限运触发 | 调 `liki-bazi` 专家 `应期判断(time_scope.type="decade")` + `liki-ziwei` 专家 `应期判断(time_scope.type="decade")`；读取合会冲宫与应期表 | 冲入宫位与触发年份 |
| 3 | 性质与走向 | 读取 `liki-bazi/skills/bazi/hehui.md` | 事件性质、结果走向、严重程度 |
| 4 | 具体细节 | 调 `liki-bazi` 专家 `本命判断(topics=["health"])` + `liki-ziwei` 专家 `本命判断(topics=["mental"])`；用户问具体年份、应期或流年时发送 `§4.1 应期判断` | 疾厄宫、福德宫、四化与流年信号 |
