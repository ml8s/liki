...
| 2 | 胜财能力 | 发送 `TOOLS.md 本命判断`；读取 `liki-bazi/skills/bazi/wealth.md` | 能担 / 不能担 / 中和 |
| 3 | 风险与运势 | 分别发送 `TOOLS.md` 的 `十年期应期判断`、本命判断 | 当前运与比劫夺财风险 |
| 4 | 具体细节或流年 | 调 `liki-bazi` 专家 `本命判断（财运）` + `liki-ziwei` 专家 `本命判断（房产）`；用户问具体年份、应期或流年时发送 `应期判断` | 紫微财库与流年财信号 |
