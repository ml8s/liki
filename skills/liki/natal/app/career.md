...
> **本卡的 本命判断 / 应期判断 调用均传 `topics=["career"]`，仅保留事业域断语。**
...
| 3 | 事业起伏 | 发送 `TOOLS.md §2 应期判断(time_scope.type="decade")` + 读取 `liki-bazi/skills/bazi/dayun.md` | 当前运与窗口年 |
| 4 | 具体细节或流年 | 调 `liki-bazi` 专家 `本命判断(topics=["career"])` + `liki-ziwei` 专家 `本命判断(topics=["relocation"])`；用户问具体年份、应期或流年时发送 `§4.1 应期判断` | 紫微事业信号与流年事件 |
...
| 用户问换工作 | 结合大运流年引动判断（发送 `TOOLS.md §3 应期判断` 查流年应期） |
