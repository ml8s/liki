...
1. 读本地 `VERSION` + `content.sha256`，再读 `https://liki.hk/skills/liki-divination/VERSION` 与 `.../content.sha256`
2. 一致 → 继续；任一不一致 → 告知更新内容，提示 `npx skills add ml8s/liki/skills/liki-divination -y`，用户确认后继续
