...
├── .mcp.json                # 连接器：engine-aux + engine-ziwei（排盘）+ counsel-ziwei（判断）
└── README.md
```

## 依赖

- `dependencies.connectors: ["engine-ziwei", "engine-aux", "counsel-ziwei"]` — 依赖排盘连接器（engine-ziwei）、历法辅助（engine-aux）与判断连接器（counsel-ziwei）。
