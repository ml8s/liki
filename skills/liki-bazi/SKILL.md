...
```
具体时刻：city_coords(城市) → 经度 → full_paipan(gregorian, 性别, 经度, correct=true)
已明确时辰：full_paipan(gregorian, 性别, correct=false) → pan
query(域, pan, [year]) → 本命断语（year 仅限大运/大限）
yearly_range(pan, 起始年, 结束年, rules) → 流年断语（rules 必填）
...
- 路 A 经度未知 → **先问用户出生地**（城市或经度）；用户给城市 → 调 `city_coords` 获取经度；仍无法获取 → **禁止按具体时刻校正排盘**（禁止用默认经度——真太阳时会偏移）
- 路 B 已明确时辰 → 经度可省略；内部仅作农历历法换算，不做真太阳时校正

_NOTE_：`correct=true` 时经度必填——禁止静默降级到默认值。city_coords 找不到城市时问用户附近的较大城市。
...
- 限运域查询会附带 `current_year/current_year_source`（指定年份标记 specified）；断言输出固定分三层：`八字`、`紫微`、`合参`；每条含 id/领域/事件类型/时间层/事件/结论，单侧信号不得伪装成跨体系结论，跨体系条件必须由 common 表命中
- 收尾约定：①排盘+用神完成后按用户问题继续领域分析或输出结论，`pan` 在当前对话上下文复用 ②给出验证时间点而用户未回应 → 结论标注「未经验证，时序判断置信度有限」③app 卡标[必读]的域文件未读**不得断具体细节**；一般卡必读不超过 3 个，命书这类整本报告卡可允许 4 个但必须注明结构性例外
...
- 步骤：① 收集候选时辰（2-3 个）+ 人生大事（3-5 件含年份）→ ② 对每个候选时辰排盘并用 `calibrate(候选列表, 事件列表)` 批量校验 → ③ 用 `domains/bazi/calibration.md` 排除不合理时辰 → ④ 用 `domains/ziwei/calibration.md` 交叉验证 → ⑤ 确定时辰（置信度高/中/低）
  - 候选给具体时刻 → `correct=true` 且必须提供出生地经度；候选已明确「X时」→ `correct=false`，经度可省略
- 无法确定 → 停止定盘并明确告知「考时证据不足」；请用户补充时辰范围或人生大事。用户明确选择某候选时，输出须标注「按用户指定候选排盘，未经考时确认」；宝宝/青少年同样不得自动代选时辰

强制规则：
1. 应期/流年分析时 yearly_range 自动附带 current_year（含 server/local 来源标注）与 year_basis（八字/紫微年界说明）；本命分析除大运/大限外无当前时间依赖
...
- 命理成员与关系读 `tools/constants.json`，因子组合读 `tools/factors/*.csv`，断语结论读 `tools/assertions/*.csv`；禁止自创规则或改写表内结论
