...
命盘报告输出前，必须按根 SKILL.md 的「结论验证」规则做历史事件校准：取 3-5 个已发生时间段，用本报告的大运/大限×流年结论反推，请用户 yes/no 验证；≥2 段吻合才可输出结论，否则回退重新审视整体解读框架（格局/用神/大运的综合取舍）后重推——历史事件验证的是整体解读框架，不能反推单一用神选择。**本卡结论验证聚焦点：不限领域（全盘各维度的应期均可验证）**。

完成命盘报告后，如果用户愿意验证，可按大运流年反推已发生的 3-5 个时间段，请用户验证是否吻合实际人生变化。反馈后回退审视整体解读框架（格局/用神/大运）调整或维持原有判断——不经由单一用神反推。
