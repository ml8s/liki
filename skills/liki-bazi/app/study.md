...
3. 大运辅助 → `query(rule=大运, year=求学关键年)`（已取得学历只在用户提供考试/毕业等关键年时回看；无关键年份不得用当前运反推历史学历）
   输出：□ 青年期大运____ 当前大运____ 对印星____

4. 学历定档 → `query(rule=十神)`
   输出：□ 学历档____ 依据____

5. 紫微合参【强制——断「具体细节」时必做，禁止只凭八字粗断】→ `query(rule=官禄)`；流年再调 `yearly_range(pan, start, end, ["yearly_study","yingqi"])` + liunian.md（文昌文曲化科、流年命宫落宫）
   输出：□ 官禄宫星曜____ 文昌文曲____ 化科____ 流年命宫____
