...
__all__ = ["load_constants", "_op", "_liu_op", "_shishen", "_resolve_tens", "_target_stars",
           "_palace_bad", "_dayun_window", "_zw_gong_op", "_OP_NAMES", "_LIU_OP_NAMES"]
...
# 算子名清单（本命/流年）——duanyu._atomic 显式分派用；新增算子须在此登记
_OP_NAMES = frozenset({
    "现", "透", "藏", "得令", "有根", "旺", "弱", "缺", "克", "生", "直读", "含", "宫含",
    "大运十神", "数量至少", "日主长生月支", "六合存在", "官杀取清", "为用", "为忌",
    "日主十干", "月支长生", "夫妻宫状态", "日支类型", "财库现", "财星入墓", "克者旺",
    "格神透", "格神根", "月令本气", "年柱官杀", "日支冲刑害", "大运窗口",
})
...
def _resolve_tens(tens, factors, gender):
    ...
    for t in tens:
        if t == "配偶星":
            ...
            result.append(t)
    return result
def _op(op: str, args, factors: dict, gender: str, chart: dict) -> "int | str":
    ...
    for key in ("liu_chong", "liu_xing", "liu_hai"):
        for rel in full.get(key, []) or []:
            if ri_zhi in (rel.get("a"), rel.get("b"), rel.get("zhi1"), rel.get("zhi2")):
                return 1
    return 0
def _liu_op(op: str, args, factors: dict, gender: str, chart: dict, ctx: Optional[dict] = None) -> int:
