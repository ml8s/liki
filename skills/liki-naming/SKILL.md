...
- **方法清单**：`bazi.chart`（排八字）/ `bazi.fullchart`（取用神）/ `qiming.pick`（取字）/ `qiming.build`（组名）/ `qiming.check`（评估）/ `qiming.char`（查现代笔画与 Wuge 笔画）
...
- **数理红线**：Wuge=true 时，五格数理按内部 Wuge 笔画由 `qiming.pick`/`qiming.check` 计算，**严禁自行推算**；候选字必须来自 `qiming.pick` 返回，禁止凭空编造
